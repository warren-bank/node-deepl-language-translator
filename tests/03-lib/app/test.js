const translate = require('../../../lib/deepl-language-translator/translate')
...
const run_test = async function(optimize_duplicates = true, use_echo_server = false) {
  const api_key              = !use_echo_server ? process.env["DEEPL_TRANSLATE_API_KEY"] : 'MY_DEEPL_TRANSLATE_API_KEY'
  const api_url              = !use_echo_server ? process.env["DEEPL_TRANSLATE_API_URL"] : 'https://httpbin.org/post#'
  ...
  try {
    ...
    if (!Array.isArray(translated_strings_array) || (translated_strings_array.length !== input_strings_array.length))
      throw new Error('ERROR: DeepL service API returned an incorrect number of string translations.')
